...
def mypokemon(request):
    if request.session.get('is_login', None):
        ...
        cursor.execute("select c_name,rarity, img,type,status from Card natural join ownedcard where userID =%s",userID)
...
def boxhistory(request):
    if request.session.get('is_login', None):
        cursor = connection.cursor()
        userID=request.session.get('userID', None)
        cursor.execute("select b_orderID, title ,pay_amount,pay_datetime from Boxorder natural join Blindbox where userID =%s",userID)
